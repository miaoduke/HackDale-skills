...
> 将原文 `【】` 占位符统一替换为：`{{TOPIC}}`（主题）、`{{CONCEPT}}`（概念）、`{{MY_ROLE}}`（你的角色）。
> 按顺序执行；每步提示词直接发给 AI 即可。
...
- 一条具体的行动建议（以我的角色{{MY_ROLE}}，我该做什么不一样的事）；
...
```
现在请你对刚才这份简报做一次同行评审——同时叠加费曼视角的批判滤镜：
...
- 有没有第 6 个视角是该加进来、加了会改变结论的；
  若【{{TOPIC}}】涉及投资 / 医疗 / 法律 / 工程安全等「建议即责任」的高风险领域，
  请从 references/domain-lens.md 载入对应领域视角，作为第 6 视角加入评审
  （按需加载，不进默认上下文；载入后仍叠加上面的费曼审查）。

**费曼视角额外审查（内置于此步，默认执行）：**
- 货物崇拜检测：简报里有没有「看起来科学、用词专业，但缺少实质验证或具体数据」的结论？如果有，逐条指出为什么它是 cargo cult 而非真知识。
- 反自欺审查：哪个结论用户最容易因为「想相信它」而选择性忽略反面证据？把那个反面证据明确摆出来。
- 命名≠理解检查：有没有用术语堆砌伪装理解的段落？试把每段核心观点翻译成六年级能懂的版本——如果翻不过去，说明那段就没说清。
```
...
请你扮演一位专家教师和技能教练，把【{{TOPIC}}】分解为 5 个
